...
/* Handles requests under the assumption that SERVER is a TPC Leader. */
static void handle_leader(server_t *server) {
  int sockfd;
  tpcleader_t *tpcleader = &server->tpcleader;
  while (server->listening) {
    sockfd = (intptr_t) wq_pop(&server->wq);
    tpcleader->handle(tpcleader, sockfd);
    close(sockfd);
  }
}

/* Handles requests under the assumption that SERVER is a kvserver follower. */
static void handle_follower(server_t *server) {
  int sockfd;
...
static void *handle(void *_server) {
  /* (Valgrind) Detach so thread frees its memory on completion, since
